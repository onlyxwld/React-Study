...
type TeamManageProps = {
  ...
  getAgreementsData: () => void;
  addPerson: (any) => void;
  updatePerson: (any) => void;
  deletePerson: (any) => void;
  suspendPerson: (any) => void;
  loadPosition: () => void;
  addPosition: (any) => void;
  loadDepartment: () => void;
  addDepartment: (any) => void;
  loadEmployees: (any) => void;
  addEmployee: (any) => void;
};
type TeamManageState = {
  columns: any[];
  isSelectedRow: boolean;
  empTableLoading: boolean;
  formatDate: string;
  edit: boolean;
  ...
  searchEmployeeData: any[];

  // position Data
  positionData: any[];
  positionName: string;
  selectedPositionColor: string;

  // modal Bool Var
  isOpenPositionModal: boolean;
  ...
  pDepartment: string;
  selectedDepartmentColor: string;
  selectedPDepartment: string;
...
class TeamManage extends Component<TeamManageProps, TeamManageState> {
  state: TeamManageState = {
    columns: [
      {
        title: "Nombre",
        dataIndex: "name",
          ...
          record.role !== "Administrador" ? (
            <Space>
              <Popconfirm
                title={
                  record.allowed
                    ? "Seguro que suspender?"
                    : "Seguro que lo permitirás?"
                }
                onConfirm={() =>
                  this.handleSuspendPerson(
                    record.key,
                    record.allowed ? false : true
                  )
                }
              >
                <a href="dashboard-team#">
                  {record.allowed ? "Suspender" : "Permitir"}
                </a>
              </Popconfirm>
              <a href="dashboard-team#" onClick={() => this.onEdit(record.key)}>
                Edit
              </a>
              <Popconfirm
                title="Seguro que quieres eliminar?"
                onConfirm={() => this.handleDeletePerson(record.key)}
              >
    ...
    // agreement data
    agreementsData: [{ value: "nobusiness", label: "nobusiness" }],

    // schedule data
    scheduleData: [{ value: "nobusiness", label: "nobusiness" }],
  ...
  handleSubmitPosition = (e) => {
    ...
    this.props.addPosition({
      ...
      color: this.state.selectedPositionColor,
    });
  ...
  handleSubmitDepartment = (e) => {
    ...
    this.props.addDepartment({
      ...
      parentId: this.state.selectedPDepartment,
      color: this.state.selectedDepartmentColor,
    });
  ...
  componentDidMount = () => {
    ...
    this.props.loadEmployees({
      ...
      company_id: this.props.companyData._id,
    ...
    if (this.props !== prevProps) {
      ...
      if (this.props.positionData && this.props.positionData.length > 0) {
        for (let i = 0; i < this.props.positionData.length; i++) {
          let obj = {
            value: this.props.positionData[i]._id,
            label: this.props.positionData[i].name,
            color: this.props.positionData[i].color,
          };
          pData.push(obj);
        }
      }
      ...
      if (this.props.departmentData && this.props.departmentData.length > 0) {
        for (let i = 0; i < this.props.departmentData.length; i++) {
          let obj = {
            value: this.props.departmentData[i]._id,
            label: this.props.departmentData[i].name,
            color: this.props.departmentData[i].color,
            parentId: this.props.departmentData[i].parentId,
          };
      ...
      if (this.props.calendarData && this.props.calendarData.length > 0) {
        for (let i = 0; i < this.props.calendarData.length; i++) {
          let obj = {
            value: this.props.calendarData[i]._id,
            label: this.props.calendarData[i].name,
          };
      ...
      if (this.props.centersData && this.props.centersData.length > 0) {
        for (let i = 0; i < this.props.centersData.length; i++) {
          let obj = {
            value: this.props.centersData[i]._id,
            label: this.props.centersData[i].name,
          };
      ...
      if (this.props.scheduleData && this.props.scheduleData.length > 0) {
        for (let i = 0; i < this.props.scheduleData.length; i++) {
          let obj = {
            value: this.props.scheduleData[i]._id,
            label: this.props.scheduleData[i].name,
          };
          scheduleData.push(obj);
        }
      }

      var agreementsData: any[] = [];
      if (this.props.agreementsData && this.props.agreementsData.length > 0) {
        for (let i = 0; i < this.props.agreementsData.length; i++) {
          let obj = {
            value: this.props.agreementsData[i]._id,
            label: this.props.agreementsData[i].name,
          };
          agreementsData.push(obj);
        }
      }

      var searchEmpData: any[] = [];
      var responsableData: any[] = [];
      if (this.props.employeeData && this.props.employeeData.length !== 0) {
        this.setRow(
      ...
      <div className="team-manage">
        <div className="team-manage-header">
          {/* <Select
                    ...
                    /> */}
          <Button type="primary" onClick={() => this.addNewPerson()}>
            Añadir persona
          </Button>
          {/* {
        ...
        <Table
          ...
          loading={this.state.empTableLoading}
        />
        <Modal
            ...
            <Form action="#" onSubmit={this.handleAddPersonSubmit}>
              ...
              <Row>
                ...
                <Col className="d-flex justify-content-center align-items-center">
                  <Button
                    type="primary"
                    onClick={() => this.positionModalToggle()}
                  >
                    Añadir
                  </Button>
                </Col>
              ...
              <Row>
                ...
                <Col className="d-flex justify-content-center align-items-center">
                  <Button
                    type="primary"
                    onClick={() => this.departmentModalToggle()}
                  >
                    Añadir
                  </Button>
                </Col>
              ...
              <Button htmlType="submit" type="primary" className="my-1 w-100">
                Aceptar
              </Button>
            </Form>
          </ModalBody>
        ...
        <Modal
          isOpen={this.state.isOpenPositionModal}
          toggle={this.positionModalToggle}
        >
          <ModalHeader toggle={this.positionModalToggle}>
            Añadir cargo
          </ModalHeader>
          <ModalBody>
            <Form action="#" onSubmit={this.handleSubmitPosition}>
              ...
              <div
                style={{
                  width: "100%",
                  height: "50px",
                  background: this.state.selectedPositionColor,
                  margin: "10px 0",
                }}
              ></div>
              <CirclePicker
                className="w-100 mb-1"
                onChangeComplete={this.onChangePositionColor}
              />
              <Button type="primary" htmlType="submit">
                Aceptar
              </Button>{" "}
              <Button type="primary" onClick={() => this.positionModalToggle()}>
                Cancelar
              </Button>{" "}
            </Form>
          </ModalBody>
        ...
        <Modal
          isOpen={this.state.isOpenDepartmentModal}
          toggle={this.departmentModalToggle}
        >
          <ModalHeader toggle={this.departmentModalToggle}>
            Departamento
          </ModalHeader>
          <ModalBody>
            <Form action="#" onSubmit={this.handleSubmitDepartment}>
              ...
              <div
                style={{
                  width: "100%",
                  height: "50px",
                  background: this.state.selectedDepartmentColor,
                  margin: "10px 0",
                }}
              ></div>
              <CirclePicker
                className="w-100 mb-1"
                onChangeComplete={this.onChangeDepartmentColor}
              />
              <Button type="primary" htmlType="submit">
                Acceptar
              </Button>{" "}
